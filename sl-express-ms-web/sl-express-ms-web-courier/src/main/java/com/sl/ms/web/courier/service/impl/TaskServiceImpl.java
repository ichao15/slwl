...
public class TaskServiceImpl implements TaskService {
    ...
    public CarriageVO calculate(CarriageCalculateVO calculateVO) {
        //1.根据县级id获取市级地址id
        List<AreaDto> areaDtos = areaFeign.findAll(null, Arrays.asList(calculateVO.getReceiverCountyId(), calculateVO.getSenderCountyId()));
        ...
        //2.将市级id作为运费计算条件
        WaybillDTO waybillDTO = ObjectUtils.convert(calculateVO, WaybillDTO.class, (calculate, waybill) -> {
        ...
        return ObjectUtils.convert(carriageDTO, CarriageVO.class, (carriage, carriageVO) -> {
     ...
     * @return 任务返回结果
     */
    private TaskVO getTaskVO(PickupDispatchTaskDTO taskDTO) {
     ...
     * @param taskVO 取派件任务
     */
    private void assembleTaskVO(TaskVO taskVO) {
        //根据订单id查询订单
        OrderDTO orderDTO = orderFeign.findById(taskVO.getOrderId());
        if (orderDTO == null) {
    ...
    private Map<String, String> getNameAndPhoneAndAddress(TaskInformationEnum taskInformationEnum, OrderDTO orderDTO) {
        ...
        switch (taskInformationEnum) {
            case SEND: //任务关联寄件人信息
                ...
                address = areaFeign.get(orderDTO.getSenderProvinceId()).getName() + areaFeign.get(orderDTO.getSenderCityId()).getName() + areaFeign.get(orderDTO.getSenderCountyId()).getName() + orderDTO.getSenderAddress();
            ...
            case RECEIVE://2任务关联收件人信息
                ...
                //根据id查询省、市、区名称并拼接为完整地址
                address = areaFeign.get(orderDTO.getReceiverProvinceId()).getName() + areaFeign.get(orderDTO.getReceiverCityId()).getName() + areaFeign.get(orderDTO.getReceiverCountyId()).getName() + orderDTO.getReceiverAddress();
    ...
    public PageResponse<TaskVO> pageQuery(TaskQueryVO vo) {
        ...
        //6.为taskVO封装订单和运单数据
        taskList.forEach(this::assembleTaskVO);
        ...
        //11.分页结果封装
        return PageResponse.<TaskVO>builder().page(vo.getPage()).pageSize(vo.getPageSize()).items(taskList).counts(pageCounts).pages(pages).build();
    }
    ...
    private void setTaskTime(Integer status, List<TaskVO> taskList) {
        if (ObjectUtil.isEmpty(status)) {
            taskList.forEach(x -> x.setTaskTime(x.getUpdated()));
        } else {
    ...
    private List<TaskVO> sortTaskList(TaskQueryVO vo, List<TaskVO> taskList) {
        if (vo.getOrderDistance() != null && vo.getOrderDistance().equals(SortOrderEnum.POSITIVE_SEQUENCE.getCode())) {
            //按距离正序
            return CollUtil.sort(taskList, Comparator.comparing(TaskVO::getDistance));
        } else if (vo.getOrderDistance() != null && vo.getOrderDistance().equals(SortOrderEnum.REVERSE_ORDER.getCode())) {
            //按距离倒序
            return CollUtil.sort(taskList, Comparator.comparing(TaskVO::getDistance).reversed());
        } else if (vo.getOrderTime() != null && vo.getOrderTime().equals(SortOrderEnum.POSITIVE_SEQUENCE.getCode())) {
            //按时间正序
    ...
    public TaskInfoVO detail(String id) {
        ...
        //根据id查询省、市、区名称并拼接为完整地址
        String senderAddress = areaFeign.get(orderDTO.getSenderProvinceId()).getName() + areaFeign.get(orderDTO.getSenderCityId()).getName() + areaFeign.get(orderDTO.getSenderCountyId()).getName() + orderDTO.getSenderAddress();
        String receiverAddress = areaFeign.get(orderDTO.getReceiverProvinceId()).getName() + areaFeign.get(orderDTO.getReceiverCityId()).getName() + areaFeign.get(orderDTO.getReceiverCountyId()).getName() + orderDTO.getReceiverAddress();
    ...
    public boolean pickup(TaskPickupVO vo) {
        ...
        if (!memberDto.getIdCardNoVerify().equals(MemberIdCardVerifyStatus.SUCCESS.getCode())) {
            ...
            TaskIdCardCheckVO taskIdCardCheckVO = BeanUtil.copyProperties(vo, TaskIdCardCheckVO.class);
            RealNameVerifyVO realNameVerifyVO = this.idCardCheck(taskIdCardCheckVO);
    ...
    public void batchTransfer(TaskBatchTransferVO dto) {
        ...
        List<PickupDispatchTaskDTO> taskDTOList = pickupDispatchTaskFeign.findByIds(idList);

        //2.批量修改取派件任务快递员id
        for (PickupDispatchTaskDTO taskDTO : taskDTOList) {
            pickupDispatchTaskFeign.updateCourierId(taskDTO.getId(), taskDTO.getCourierId(), Long.valueOf(dto.getAnotherCourierId()));
        }
    }
    ...
    public TaskStatisticsVO taskStatistics() {
        ...
        //今日取件任务数量
        taskStatisticsVO.setPickupNum(pickupDispatchTaskFeign.todayTasksCount(userId, PickupDispatchTaskType.PICKUP, null, PickupDispatchTaskIsDeleted.NOT_DELETED));

        //今日待取件任务数量
        taskStatisticsVO.setNewPickUpNum(pickupDispatchTaskFeign.todayTasksCount(userId, PickupDispatchTaskType.PICKUP, PickupDispatchTaskStatus.NEW, PickupDispatchTaskIsDeleted.NOT_DELETED));

        //今日已取件任务数量
        taskStatisticsVO.setCompletePickUpNum(pickupDispatchTaskFeign.todayTasksCount(userId, PickupDispatchTaskType.PICKUP, PickupDispatchTaskStatus.COMPLETED, PickupDispatchTaskIsDeleted.NOT_DELETED));

        //今日已取消取件任务数量
        taskStatisticsVO.setCancelPickUpNum(pickupDispatchTaskFeign.todayTasksCount(userId, PickupDispatchTaskType.PICKUP, PickupDispatchTaskStatus.CANCELLED, PickupDispatchTaskIsDeleted.NOT_DELETED));

        //今日派件任务数量
        taskStatisticsVO.setDispatchNum(pickupDispatchTaskFeign.todayTasksCount(userId, PickupDispatchTaskType.DISPATCH, null, PickupDispatchTaskIsDeleted.NOT_DELETED));

        //今日待派件任务数量
        taskStatisticsVO.setNewDispatchNum(pickupDispatchTaskFeign.todayTasksCount(userId, PickupDispatchTaskType.DISPATCH, PickupDispatchTaskStatus.NEW, PickupDispatchTaskIsDeleted.NOT_DELETED));

        //今日已签收任务数量
        taskStatisticsVO.setSignedNum(pickupDispatchTaskFeign.todayTasksCount(userId, PickupDispatchTaskType.DISPATCH, PickupDispatchTaskStatus.COMPLETED, PickupDispatchTaskIsDeleted.NOT_DELETED));

        //今日已取消派件任务数量
        taskStatisticsVO.setCancelDispatchNum(pickupDispatchTaskFeign.todayTasksCount(userId, PickupDispatchTaskType.DISPATCH, PickupDispatchTaskStatus.CANCELLED, PickupDispatchTaskIsDeleted.NOT_DELETED));
    ...
    public PageResponse<TaskVO> search(TaskSearchVO taskSearchVO) {
        ...
        //4.将订单、运单相关数据封装
        List<TaskVO> taskVOList = pickupDispatchTaskDTOS.stream().map(this::getTaskVO).collect(Collectors.toList());
        ...
        //8.分页结果封装
        return PageResponse.<TaskVO>builder().page(pageResponse.getPage()).pageSize(pageResponse.getPageSize()).items(taskVOList).counts(pageResponse.getCounts()).pages(pageResponse.getPages()).build();
    }
}
